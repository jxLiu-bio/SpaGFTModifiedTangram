...
class Mapper:
    ...
    def __init__(
        ...
        self.lambda_g4 = lambda_g4
        self.lambda_r = lambda_r
        self.learning_rate = learning_rate
        self._density_criterion = torch.nn.KLDivLoss(reduction="sum")
        if fm_mtx is not None:
            self.fm_mtx = torch.tensor(fm_mtx, device=device, dtype=torch.float32)
    ...
    def _loss_fn(self, verbose=True):
        ...
        if self.lambda_g3 != 0 or self.lambda_g4 != 0:
            normed_G_pred = torch.nn.functional.normalize(G_pred, p=2, dim=0)
            normed_G = torch.nn.functional.normalize(self.G, p=2, dim=0)
            fv_term = self.lambda_g3 * cosine_similarity(self.fm_mtx @ normed_G_pred @ normed_G_pred.transpose(0, 1),
                                                         self.fm_mtx @ normed_G @ normed_G.transpose(0, 1),
                                                         dim=0).mean()
            vf_term = self.lambda_g4 * cosine_similarity(self.fm_mtx @ normed_G_pred,
                                                         self.fm_mtx @ normed_G,
                                                         dim=0).mean()

            expression_term = gv_term + vg_term + fv_term + vf_term
        else:
