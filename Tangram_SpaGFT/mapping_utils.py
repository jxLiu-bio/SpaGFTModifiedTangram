...
def map_cells_to_space_gft(
    ...
    # Choose device
    device = torch.device(device)  # for gpu
    # obtain fourier modes using spatial info
    if lambda_g3 != 0 and spatial_key is None:
        raise  KeyError("spatial information should be provided")

    lap_mtx = get_laplacian_mtx(adata_sp,
                                spatial_key=spatial_key,
                                num_neighbors=int(ratio_neighbors * np.sqrt(adata_sp.shape[0])),
                                normalization=False)
    v0 = [1 / np.sqrt(adata_sp.shape[0])] * adata_sp.shape[0]
    # Fourier modes of low frequency
    num_low_frequency = np.ceil(ratio_fms * np.sqrt(adata_sp.shape[0]))
    num_low_frequency = int(num_low_frequency)
    num_low_frequency = min(adata_sp.shape[0] - 1,
                            num_low_frequency)
    eigvals, eigvecs = ss.linalg.eigsh(lap_mtx.astype(float),
                                       ...
                                       v0=v0)
    c = 0.001
    eig_power = [1 / (1 + c * eigval) for eigval in eigvals]
    eigvecs = eig_power * eigvecs
    ...
    if mode in ["cells", "clusters"]:
        hyperparameters = {
            ...
            "lambda_g1": lambda_g1,  # gene-voxel cos sim
            "lambda_g2": lambda_g2,  # voxel-gene cos sim
            "lambda_g3": lambda_g3, # fm-voxel cos sim
            "lambda_g4": lambda_g4,       # voxel-fm cos sim
            "lambda_r": lambda_r,  # regularizer: penalize entropy
            "d_source": d_source,
            "fm_mtx": eigvecs,
        }
